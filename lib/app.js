...
app.post('/api/webhook', function(req, res, next) {
	req.pipe(concat(function(buf) {
		...
		// TODO: this is a dirty hack because of yargs/yargs#716 on GitHub
		var override = argv['override.' + repoName];
		if (override) {
			dir = path.normalize(argv.path + '/' + override);
		} else {
			dir = path.normalize(argv.path + '/' + repoName);
		}
